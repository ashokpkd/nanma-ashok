...
const ResetForm = () => {
    const { data, loading, errors: apiErrors, postData } = useCustomPostApi({url:'https://portal.umall.in/api/check-mobile'})
    const { data:resetData, loading:loadingMsg, errors: resetErrors, postData:resetPassword }  =useCustomPostApi({url:'https://portal.umall.in/api/reset-password',successCB:handleOnReset})
    ...
    const handleOnsubmit = async(event) => {
        ...
        // if (value.enterOtp !== data.otp) {
        //     newErrors.otpNotSame = 'OTP is invalid'
        // }
  ...
  return (
      <>
          <form action="" className='w-full flex flex-col gap-5'>
              ...
              <Button name={'Get OTP'} className={"rounded-xl h-10 text-xs  font-semibold"} onClick={handleOnSendOtp}/> 
              </div>
              <div>
             ...
             {click ? <RiEyeCloseLine
              ...
              <div className=' -mt-5'>
              {errors.resetPassword && <span className="text-xs text-red-600 ">{errors.resetPassword}</span> }                 
              </div>
              <div>
              ...
              <div>
                  <Button name={'Reset Password'} className={'font-semibold '} onClick={handleOnsubmit}/>
              </div>
      </form>
      </>
    )
